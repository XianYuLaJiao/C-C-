...
class Linear(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, bias=True, w_init='linear'):
        """
        :param in_dim: dimension of input # 输入维度
        :param out_dim: dimension of output  # 输出维度
        :param bias: boolean. if True, bias is included.  布尔类型，参数bias
        :param w_init: str. weight inits with xavier initialization.
        """
        super().__init__()  # Linear, self
...
class MultiheadAttention(nn.Module):  # 点注意力机制
    """
...
class Linear(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, bias=True, w_init='linear'):
        """
        :param in_dim: dimension of input 输入张量size
        :param out_dim: dimension of output 输出质张量size,全连接层
        ...
        nn.init.xavier_uniform_(
            self.linear_layer.weight,
            gain=nn.init.calculate_gain(w_init)) # 均匀分布 

    def forward(self, x):
        return self.linear_layer(x)
```

![c062fc3aff64723a7950b0b1648d618](C:\Users\17180\AppData\Local\Temp\WeChat Files\c062fc3aff64723a7950b0b1648d618.png)
